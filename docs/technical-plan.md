...
`handleBrickCollisions()` 在同一帧内按球逐个处理砖块碰撞。每颗球最多命中一个砖块，命中后跳出该球的砖块扫描，但不会中断其他球的碰撞处理，保证多球道具在高密度关卡中有稳定清场反馈。钢铁砖块使用连续碰撞检测（swept collision），沿球体运动轨迹分段检测，防止高速弹球在一帧内穿过钢铁砖块而不被拦截。
